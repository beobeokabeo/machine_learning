...
def csv_to_dataset(csv_path):
    ...
    sns.lineplot(x=data.index, y='1. open', data=data)
...
def btc_csv_to_dataset(csv_path):
    data = pd.read_csv(csv_path)
    df = pd.read_csv(csv_path, names=['time', 'low', 'high', 'open', 'close', 'volume'], parse_dates=['time'])
    ...
    data.to_csv('./BTC_1h_resampled.csv')
